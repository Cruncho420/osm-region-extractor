...
## Pinned timezone source-local generation

Retry [34824871686](https://github.com/Cruncho420/osm-region-extractor/actions/runs/34824871686) passed dependency configuration, then failed during compilation because `src/baldr/CMakeLists.txt:13-17` invokes `make leapseconds` in `third_party/tz`. At the pinned timezone revision this writes temporary `leapseconds.out` then renames it to ignored, untracked `leapseconds`; neither is a tracked source input. The read-only mount correctly refused that write.

The workflow now copies only the small timezone submodule to the ephemeral runner build directory and overlays that writable copy at its original path; every other source path remains read-only. Git metadata still resolves to the authoritative source checkout. The existing exact core/gitlink/dirty tracked-source checks remain unchanged before/after build. An additional ignored-inclusive file inventory requires no generated files initially and exactly `leapseconds` after success, rejects leftover `.out` or any extra file, retains its SHA256/size in the build receipt, and checks that hash again after graph generation. No source mutation exception or broad dirty-source waiver is added.

The exact-core custom-command audit found locale, embedded Lua, timezone and Windows-zone header outputs targeting the binary directory; protobuf/Python configuration outputs for this enabled build also target the binary directory. The Python binding version file writes into source only with Python bindings enabled, which this job disables. The pinned leap-second Makefile rule is the identified enabled source-local write. This correction has a fail-first generated-file contract regression and local syntax/unit validation only; another reviewed remote run is required. No local native compilation or copied source tree was created.
