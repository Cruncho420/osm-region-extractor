...
async function main(): Promise<void> {
  ...
  const results = await runPool(regions, concurrency, ([regionId, region]) =>
  ...
  if (failures.length > 0) {
    ...
    for (const f of failures) {
      ...
      console.error(`::error title=Region ${f.regionId} ${f.status}::${f.regionId}.sqlite.gz failed integrity (${f.status}): ${f.detail}`);
    }
    console.error(
